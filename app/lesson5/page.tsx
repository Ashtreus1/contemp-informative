...
function Section1() {
	return (
		<Section className="h-screen flex flex-col lg:flex-row gap-4 py-20 md:gap-8 bg-[#303030]">
			...
			<RightAnim className="w-full lg:w-1/2 flex flex-col justify-center p-4 xl:p-20 text-left font-mont gap-5 text-white" duration={duration}>
...
function Template1({ item, index }: any) {
	return (
		<Section className={``}>
			...
			<RightAnim values="50" className={`${articleStyle} `} duration={duration}>
...
function Template2({ item, index }: any) {
	return (
		<Section className={`flex-col-reverse bg-[#303030] sm:bg-[#1e1e1e]`}>
			<LeftAnim values="50" className={articleStyle} duration={duration}>
				<h1 className={`${titleStyle}`}>{item.title}</h1>
				{item.article.map((i: any) => (
